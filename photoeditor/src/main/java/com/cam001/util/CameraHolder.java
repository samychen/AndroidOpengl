...
public class CameraHolder {
    ...
    public synchronized android.hardware.Camera open(int cameraId)
        ...
        if (mCameraDevice == null) {
            try {
                LogUtil.logV(TAG, "open camera " + cameraId);
                mCameraDevice = android.hardware.Camera.open(cameraId);
                mCameraId = cameraId;
            } catch (RuntimeException e) {
            	LogUtil.logE(TAG, "fail to connect Camera", e);
            ...
            try {
            	...
            	LogUtil.logE(TAG, "reconnect failed.");
                throw new CameraHardwareException(e);
            }
    ...
    private synchronized void releaseCamera() {
    	CameraUtil.Assert(mUsers == 0);
    	CameraUtil.Assert(mCameraDevice != null);
    ...
    public synchronized void keep() {
    	...
    	CameraUtil.Assert(mUsers == 1 || mUsers == 0);
