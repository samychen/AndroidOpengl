...
public class CropImageView extends FrameLayout {
	...
	public CropImageView(Context context, AttributeSet attrs) {
		...
		init(context);
		mMicroPadding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				PaintUtil.DEFAULT_LINE_THICKNESS_DP, context.getResources()
						.getDisplayMetrics());
	}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (mBitmap != null) {
			final Rect bitmapRect = ImageViewUtil.getBitmapRectCenterInside(
					mBitmap, this, mCropOverlayView.strokwidth);
			// transformRect(bitmapRect);
			mCropOverlayView.setBitmapRect(bitmapRect);
			FrameLayout.LayoutParams param = (LayoutParams) mImageView
					.getLayoutParams();
			param.width = bitmapRect.width();
			param.height = bitmapRect.height();
	...
	// private void transformRect(Rect bitmapRect){
	// mMicroPadding *= 5;
	// if(bitmapRect.left == 0){
	// bitmapRect.left += mMicroPadding;
	// bitmapRect.right -= mMicroPadding;
	// int h = (int)
	// ((bitmapRect.width()-2*mMicroPadding)*bitmapRect.height()/bitmapRect.width());
	// int adjustH = (bitmapRect.height()-h)/2;
	// bitmapRect.top += adjustH;
	// bitmapRect.bottom -= adjustH;
	// }
	// if(bitmapRect.top == 0){
	// bitmapRect.top += mMicroPadding;
	// bitmapRect.bottom -= mMicroPadding;
	// int w = (int)
	// ((bitmapRect.height()-2*mMicroPadding)*bitmapRect.width()/bitmapRect.height());
	// int adjustW = (bitmapRect.width()-w)/2;
	// bitmapRect.left += adjustW;
	// bitmapRect.right -= adjustW;
	// }
	//
	// }
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mBitmap != null) {
			...
			final Rect bitmapRect = ImageViewUtil.getBitmapRectCenterInside(
					...
					mLayoutHeight, mCropOverlayView.strokwidth);
			mCropOverlayView.setBitmapRect(bitmapRect);
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		if (mCropOverlayView != null) {
			...
			mCropOverlayView.setImageBitmap(mBitmap);
		}
	}
	...
	public RectF getTransformRectF(int orWidth, int orHeight){
		RectF dst = new RectF();
		Matrix matrix = new Matrix();
		matrix.setRectToRect(new RectF(0, 0, mBitmap.getWidth(), mBitmap.getHeight()), new RectF(0, 0, orWidth, orHeight), ScaleToFit.FILL);
		RectF src = getCropRectF();
	...
	public RectF getCropRectF(){
		...
		int left = (int) actualCropX > 0?(int) actualCropX:0;
		int top = (int) actualCropY > 0?(int)actualCropY:0;
		return new RectF(left, top, left+(int) actualCropWidth, top+(int) actualCropHeight);
	}
	/**
	...
	public Bitmap getCroppedImage() {
		...
		/*if (((int) actualCropWidth) % 4 != 0
				|| ((int) actualCropHeight) % 4 != 0) {// when 4x crash
			int width = ((int) actualCropWidth) / 4 * 4;
			int height = ((int) actualCropHeight) / 4 * 4;
			if(width/height == 1)//may 1x*1x crash
				height -= 1;
			croppedBitmap = Bitmap.createBitmap(mBitmap, (int) actualCropX,
					(int) actualCropY, width, height);

		} else{
			if(((int) actualCropWidth)/((int) actualCropHeight) == 1){//may 1x*1x crash
				croppedBitmap = Bitmap.createBitmap(mBitmap, (int) actualCropX,
						(int) actualCropY, (int) actualCropWidth,
						((int) actualCropHeight)-1);
			}else
				croppedBitmap = Bitmap.createBitmap(mBitmap, (int) actualCropX,
						(int) actualCropY, (int) actualCropWidth,
						(int) actualCropHeight);
		}*/
		croppedBitmap = Bitmap.createBitmap(mBitmap, (int) actualCropX > 0?(int) actualCropX:0,
				(int) actualCropY > 0?(int)actualCropY:0, (int) actualCropWidth,
				(int) actualCropHeight);
