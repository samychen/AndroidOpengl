    ...
    public void draw(){
        ...
        if(size>0){
            for (AFilter filter:mFilters){
                ...
                //note:为framebuffer挂载fTexute[textureIndex%2]来存储颜色，绑定后绘制会绘制到fTexure[textureIndex%2]上了
                GLES20.glFramebufferTexture2D(GLES20.GL_FRAMEBUFFER, GLES20.GL_COLOR_ATTACHMENT0,
                        GLES20.GL_TEXTURE_2D, fTexture[textureIndex%2], 0);
                GLES20.glFramebufferRenderbuffer(GLES20.GL_FRAMEBUFFER, GLES20.GL_DEPTH_ATTACHMENT,
                        GLES20.GL_RENDERBUFFER, fRender[0]);
                GLES20.glViewport(0,0,width,height);
                Log.e("GroupFilter", "textureIndex: "+textureIndex );
                if(textureIndex==0){
                    filter.setTextureId(getTextureId());//第一个滤镜必须设置为特效处理Filter的getOutputTexture，也即 fTexture[0]
                } else {
                    filter.setTextureId(fTexture[(textureIndex-1)%2]);//剩下的fTexure[(textureIndex-1)%2]设置为
                }
                filter.draw();
                unBindFrame();
                textureIndex++;
