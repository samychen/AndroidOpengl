...
import com.learnopengles.android.FileLogger;
                ...
                synchronized (sGLThreadManager) {
                    ...
                    while (true) {
                        ...
                        if ((!mHasSurface) && (!mWaitingForSurface)) {
                            Log.e(TAG, "noticed surfaceView surface lost tid=" + getId());
                            if (mHaveEglSurface) {
                                stopEglSurfaceLocked();
                            }
                        ...
                        if (mHasSurface && mWaitingForSurface) {
                            Log.e(TAG, "noticed surfaceView surface acquired tid=" + getId());
...
//                            Log.i(TAG, "sending render notification tid=" + getId());
                        ...
                        if (readyToDraw()) {
                            ...
                            if (mHaveEglSurface) {
                                if (mSizeChanged) {
                                    ...
                                    Log.e(TAG, "noticing that we want render notification tid=" + getId());
                ...
                if (mChoreographerRenderWrapper.canSwap()) {
                    ...
                    switch (swapError) {
                        ...
                        case EGL11.EGL_CONTEXT_LOST:
                            Log.e(TAG, "egl context lost tid=" + getId());
                            lostEglContext = true;
                            break;
                        default:
    ...
    public void surfaceCreated() {
        synchronized (sGLThreadManager) {
            Log.e(TAG, "surfaceCreated tid=" + getId());
            mHasSurface = true;
    ...
    public void onWindowResize(int w, int h) {
        synchronized (sGLThreadManager) {
            ...
            while (!mExited && !mPaused && !mRenderComplete
                    && ableToDraw()) {
                Log.e(TAG, "onWindowResize waiting for render complete from tid=" + getId());
                try {
                    sGLThreadManager.wait();
                } catch (InterruptedException ex) {
                    Thread.currentThread().interrupt();
    ...
    private static class GLThreadManager {
        ...
        public synchronized void threadExiting(GLThread thread) {
            Log.e(TAG, "exiting tid=" + thread.getId());
            thread.mExited = true;
            if (mEglOwner == thread) {
                mEglOwner = null;
            }
