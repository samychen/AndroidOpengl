			...
			try {
				...
				for (int y = 0; y < yLength; y++) {
					for (int x = 0; x < xLength; x++) {
						...
						//z轴除以10倍，x和y轴也需要除以10倍，否则normal矩阵不正确
						final float xSlope = (2 * xPosition) / 10f;//z对x求导
						final float ySlope = (2 * yPosition) / 10f;//z对y求导
				...
				final int numStripsRequired = yLength - 1;//从第二行开始
				final int numDegensRequired = 2 * (numStripsRequired - 1);//退化三角形点
				final int verticesPerStrip = 2 * xLength;//一次读取两行[1,2],[2,3],[3,4]。。。
				final short[] heightMapIndexData = new short[(verticesPerStrip * numStripsRequired) + numDegensRequired];
				offset = 0;
				for (int y = 0; y < yLength - 1; y++) {
					//如果我们在第二行或后续行上，我们将复制该行的第一个顶点，如果我们在最后一行上，我们也会复制该行的第一个顶点
					if (y > 0) {
