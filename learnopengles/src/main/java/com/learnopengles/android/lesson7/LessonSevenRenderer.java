    ...
    class GenDataRunnable implements Runnable {
        ...
        public void run() {
            try {
                ...
                //以mRequestedCubeFactor为例，x1,x2取值分别有[-1,-3/5],[-1/5,1/5],[3/5,1]
...
//						Toast.makeText(mLessonSevenActivity, "Out of memory; Dalvik takes a while to clean up the memory. Please try again.\nExternal bytes allocated=" + dalvik.system.VMRuntime.getRuntime().getExternalBytesAllocated(), Toast.LENGTH_LONG).show();								
    ...
    class CubesWithVbo extends Cubes {
        ...
        CubesWithVbo(float[] cubePositions, float[] cubeNormals, float[] cubeTextureCoordinates, int generatedCubeFactor) {
            ...
            //cubePositionsBuffer.capacity（）* BYTES_PER_FLOAT：此缓冲区应包含的字节数
            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, buffers[0]);//VAO
            ...
            /**
             * {@link http://www.songho.ca/opengl/gl_vbo.html}
             *  像glBufferDataARB（）一样，glBufferSubDataARB（）用于将数据复制到VBO中，但它只是从给定的偏移量开始，将一定范围
             *  的数据替换到现有的缓冲区中。（在使用glBufferSubDataARB（）之前，缓冲区的总大小必须由glBufferDataARB（）设置。）
             *  final int buffers2[] = new int[1];
             *  GLES20.glGenBuffers(1, buffers2, 0);
             *  GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, buffers2);
             *  GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, cubePositionsBuffer.capacity() * BYTES_PER_FLOAT+
             *  cubeNormalsBuffer.capacity() * BYTES_PER_FLOAT+cubeTextureCoordinatesBuffer.capacity() * BYTES_PER_FLOAT, 0, GLES20.GL_STATIC_DRAW);
             *  GLES20.glBufferSubData(GLES20.GL_ARRAY_BUFFER,0,cubePositionsBuffer.capacity() * BYTES_PER_FLOAT,cubePositionsBuffer);
             *  GLES20.glBufferSubData(GLES20.GL_ARRAY_BUFFER,cubePositionsBuffer.capacity() * BYTES_PER_FLOAT,
             *  cubeNormalsBuffer.capacity() * BYTES_PER_FLOAT,cubeNormalsBuffer);
             *  GLES20.glBufferSubData(GLES20.GL_ARRAY_BUFFER,cubePositionsBuffer.capacity() * BYTES_PER_FLOAT+
             *  cubeNormalsBuffer.capacity() * BYTES_PER_FLOAT,cubeTextureCoordinatesBuffer.capacity() * BYTES_PER_FLOAT,cubeTextureCoordinatesBuffer);
             */
        ...
        public void render() {
            ...
            GLES20.glEnableVertexAttribArray(mNormalHandle);
            GLES20.glVertexAttribPointer(mNormalHandle, NORMAL_DATA_SIZE, GLES20.GL_FLOAT, false, 0, 0);//使用单独的缓冲区，因此偏移量为0

            // Pass in the texture information
            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, mCubeTexCoordsBufferIdx);
            GLES20.glEnableVertexAttribArray(mTextureCoordinateHandle);
            GLES20.glVertexAttribPointer(mTextureCoordinateHandle, TEXTURE_COORDINATE_DATA_SIZE, GLES20.GL_FLOAT, false,
                    0, 0);//使用单独的缓冲区，因此偏移量为0
